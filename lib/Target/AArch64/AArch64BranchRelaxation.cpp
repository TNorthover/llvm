...
bool AArch64BranchRelaxation::fixupConditionalBranch(MachineInstr &MI) {
  ...
  if (FBB && isBlockInRange(MI, *FBB)) {
    ...
    int OldSize = 4 * TII->RemoveBranch(*MBB);
    int NewSize = 4 * TII->InsertBranch(*MBB, FBB, TBB, Cond, DL);
    ...
    NewBBSize += 4 * TII->InsertUnconditionalBranch(NewBB, FBB, DL);
  ...
  TII->ReverseBranchCondition(Cond);
  MBBSize -= 4 * TII->RemoveBranch(*MBB);
  MBBSize += 4 * TII->InsertBranch(*MBB, &NextBB, TBB, Cond, DL);
